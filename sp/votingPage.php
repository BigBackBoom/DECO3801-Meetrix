    	...
    	<?php
			...
			if(isset($_POST['answer'])){
				...
				if(strtotime($current) > strtotime($end)){
					$_SESSION["votingId"] = $votingId;
					header("Location: viewVotingPage.php");
				}
			}
	...
	<body>
		...
		<div id="votingPopUp">
			<form action="votingPage.php" id="edit" method="post">
				<?php
					echo "<div id='title' style='margin-bottom: 20px; background-color:rgba(0, 233, 0, 0.4);'>";
					echo "<h3 style='width: 100%; font-family:sans-serif; display:block; font-size:16px; text-align: center; padding: 10px;'>". $posts[0]['question'] ."</h3>";
					echo "<p id='timeleft' style='padding-bottom: 10px;width: 100%; font-family:sans-serif; display:block; font-size:13px; text-align: center'>Time Left: </p>";
					echo "</div>";
					foreach($posts as $post){
						echo "<input style='margin-left: 20px;' type='radio' name='answer'  value='". $post['result_id']."'>". $post['answer'];
						echo "<br/>";	
					}
					echo "<input style='display:block; margin-top:10px; margin-left: auto; margin-right: 20px;' type='submit' value='Submit'>";
				?>
			</form>	
		</div>
	</body>
